  ...
  allowSupressLogLevel:(BOOL)allowSupressLogLevel
{
    ...
    // default values
    [self setLogLevel:ADJLogLevelInfo];
...
- (void)setLogLevelWithEnvironment:(NSString *)environment {
    [self setLogLevel:self.logLevel environment:environment];
}
...
- (BOOL)checkEnvironment:(NSString *)environment
{
    ...
    if ([environment isEqualToString:ADJEnvironmentSandbox]) {
        [self setLogLevelWithEnvironment:environment];
